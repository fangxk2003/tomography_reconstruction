...
void applyFilter(double* data, int size) {
    ...
    // vector<double> paddedData(2 * size, 0.0);
    // for (int i = 0; i < size; ++i) {
    //     paddedData[i] = data[i];
    // }
    // size *= 2; // Update size to reflect the zero-padded length
    ...
    for (int i = -N; i < N; ++i) {
        for (int j = -N; j < N; ++j) {
            ...
            for (int d = 0; d < Directions; ++d) {
                ...
                double weight = 1.0 / (y_prime * y_prime + (x_prime - sqrt(2)) * (x_prime - sqrt(2)));
                // cout << gamma << endl;
                if (gamma >= 0 && gamma < Intervals - 1) {
                    int intgamma = static_cast<int>(gamma);
                    // cout << attenuation[d][intgamma] << " " << attenuation[d][intgamma + 1] << " " << d << " " << intgamma << endl;
                    sum += (attenuation[d][intgamma] * (intgamma + 1 - gamma) + attenuation[d][intgamma + 1] * (gamma - intgamma));
                }
            }
            reconstructed[i + N][j + N] = sum;
            // cout << sum << endl;
        }
    }
    outputImage(reinterpret_cast<double*>(reconstructed), "./images/reconstructed_" + to_string(Directions) + "d_" + to_string(Intervals) + "i_8bit_fanbeam_amp.png", true);
    // outputImage(reinterpret_cast<double*>(reconstructed), "./images/reconstructed_" + to_string(Directions) + "d_" + to_string(Intervals) + "i_8bit_fanbeam.png");
    return 0;
}